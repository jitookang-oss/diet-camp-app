...
### 약사 메시지 카드
- 관리자가 보낸 미확인 메시지가 있으면 식사 사진 아래 `💌 이보라 약사님의 메시지` 접이식 카드 자동 표시
- 읽음 = 삭제 방식: "확인했어요 !!" 버튼 누르면 해당 메시지 DB에서 삭제, 모두 확인 시 카드 사라짐
- 미확인 메시지 수 빨간 뱃지 표시, 새 메시지 있으면 자동 펼쳐짐
- 메시지는 최근 30일 이내 미삭제 건만 조회 (별도 만료 크론 없음)
...
- **참여자 목록 탭:**
  ...
  - 참여자 삭제 버튼
- **체크인 현황 탭:**
  - **이번 주 참여 현황 카드 (상단):** 참여자별 2열 카드 — 월~일 요일 바(초록=완료/분홍=미완료/회색=미래), N/7일, N/총항목, 달성률%, 진행 바
  - 날짜 선택 → 당일 체크인 내역 조회 (영양제/점심걷기/저녁운동/주간미션)
  - 해당 날짜 아침 식사 사진 그리드 표시 (업로드한 참여자만) — 클릭 시 확대
  - 일별 테이블 모바일 개선: 이름 컬럼 sticky, 전화번호 모바일 숨김, 헤더 아이콘 전용
  - **메시지 발송 (하단):** 전체/개인 발송 선택 → 텍스트 입력 → 발송. 미확인 메시지 목록 표시
- **로그인:** 비밀번호 확인 후 localStorage에 인증 상태 저장 → 재방문 시 자동 로그인. 헤더 로그아웃 버튼으로 초기화 가능
...
created_at (timestamptz)
```
- 읽음 = 삭제 방식이므로 `is_read` 컬럼 없음
- 전체 발송 시 참여자 수만큼 개별 행 생성
...
│       ├── message/route.ts          # 약사 메시지 API (GET조회/POST발송/DELETE확인삭제)
...
| 관리자 체크인 탭 — 이번 주 참여 현황 카드 + 모바일 테이블 개선 | 2026-06-11 |
| 약사 → 참여자 메시지 기능 (전체/개인 발송, 읽음=삭제, 체크인 페이지 카드) | 2026-06-12 |
| 관리자 로그인 자동 유지 (localStorage) + 로그아웃 버튼 | 2026-06-12 |
...
### 보류 중 — 추후 재검토 (2026-06-11 논의)

아래 3가지 기능은 페이지 복잡도 증가 우려 + 캠프 진행 상황 지켜보며 시점 조율하기로 결정.

| 기능 | 설계 방향 | 보류 이유 |
|------|-----------|-----------|
| 🔥 연속 체크인 스트릭 | 체크인 페이지 헤더에 "N일 연속" 표시. 1항목 이상 = 달성 기준 | 체크인 페이지 과부하 우려, 추후 재검토 |
| 📊 체중 변화 그래프 | 4주차 이후 + 데이터 2주 이상 있을 때만 체크인 페이지에 표시. weekly_records 기반 | 4주차(2026-06-29) 이후 의미 있음, 그 전엔 데이터 부족 |
| 🏆 이번 주 등수 메시지 | 항목별 1~3등에게만 노출. "N등이에요" + 격려 멘트. 서버 API로 타인 데이터 비노출 처리. 주 단위 자동 리셋 | 참여자 수 적으면 희소성 없음, 동률 처리 미정, 역효과 가능성 검토 필요 |
