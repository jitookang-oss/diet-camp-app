...
export async function GET(request: Request) {
  ...
  const templateId = process.env.SOLAPI_TEMPLATE_LUNCH;
  ...
  for (const p of participants ?? []) {
    if (!p.phone) continue;
    const token = generateCheckinToken(p.phone, today, "lunch_walk");
    const url = `${baseUrl}/checkin?type=lunch_walk&date=${today}&phone=${encodeURIComponent(p.phone)}&token=${token}`;
    try {
      await sendAlimtalk({
        to: p.phone,
        templateId,
